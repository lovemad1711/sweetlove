...
class WarehouseTab(QWidget):
    ...
    def init_ui(self):
        ...
        input_group = QGroupBox('입고 데이터 입력')
    ...
    def add_entry(self):
        ...
        try:
            self.main_window.db.add_warehouse_entry(date, product_id, boxes, notes)
            self.main_window.show_success('입고 데이터가 추가되었습니다')
            self.clear_form()
            self.load_history()
    ...
    def load_history(self):
        inventory = self.main_window.db.get_warehouse_inventory()
